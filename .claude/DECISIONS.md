...
## 2026-07-07: New-SDI rerun → vintage 20260706; deliverables shipped date-stamped, canonical unchanged
**Decision:** Reran M1 (`6004b730…`) consolidated predictions with SDI `s130v89` — both past (`/mnt/share/forecasting/data/32/past/sdi/past_sdi_s130v89/sdi.nc`) and future (`…/32/future/sdi/future_sdi_s130v89/sdi.nc`) replaced the previous s130v66 pair (note: the past file changed too, not just future). New vintage `04-predict/20260706/` (`<mid>_a0`, `_a1`, `_blend`, `_blend_sr31guard`); draws/fit untouched — SDI enters at prediction time only. `run-finalize-deliverable` run twice: default (SR-31 hard-zero, = June behavior) and `--sr31-guard` (SR-31 ratio = 0.1 × smallest non-zero = 0.00385) into a separate `--out-dir`. Shipped to `direct_risk/` as `20260707_adjusted_direct_deaths.{parquet,nc}` + `20260707_sr31guard_adjusted_direct_deaths.{parquet,nc}` (chmod 775, no overwrites).
**Why:** Date-stamping ships both variants for comparison without overwriting the June deliverable while the SR-31 guard question is open; consumers of the unversioned `direct_deaths.*` are unaffected until the decision lands. Comparison plots: `notebooks/20260707/sr_version_comparison.ipynb`.
**Revisit if:** The SR-31 decision is made → promote one variant to the canonical `direct_deaths.{parquet,nc}` names.

## 2026-07-07: jobmon_installer_ihme pinned to 10.12.2
**Decision:** Upgraded `jobmon_installer_ihme` 10.11.6 → 10.12.2; pin updated in idd-tc-mortality's pyproject (idd-tools already at 10.12.2); both editable installs refreshed with `pip install -e . --no-deps`.
**Why:** The cluster's Slurm upgrade removed the slurmrestd `v0040` REST endpoint; every jobmon array submit failed with 404 at `slurm_v0040_post_job_submit`. 10.12.2's slurm plugin targets the endpoint the upgraded cluster serves.
**Revisit if:** The next cluster Slurm upgrade breaks submits the same way — check `pip list | grep -i jobmon` against IHME infra's recommended installer version first.

## 2026-07-13: super-region rake generalized to mean OR median (default stays median)
**Decision:** The super-region rake (finalize step (b)) can now match either central-tendency statistic. `super_region_median_ratios` renamed `super_region_ratios` with a `statistic="median"|"mean"` param (`.agg(statistic)` over the obs window for both obs and pred); `run-finalize-deliverable` gained `--rake-stat` (mean suffixes the adjusted file `_mean`, default median stays byte-identical to the shipped file). New `scripts/build_sr_version_files.py` reads the unadjusted blend once and emits the 2(stat)×2(guard)+unadjusted matrix as date-prefixed files. Shipped the 5-version set for M1/20260706 to `direct_risk/` as `2026_07_13_*_direct_deaths.{parquet,nc}`; `notebooks/20260707/sr_version_comparison.ipynb` now plots all 5 as columns.
**Why:** Bobby wanted mean-raked deliverables alongside median for comparison. For heavy-tailed TC deaths the two diverge sharply — mean ratios are far larger (SR4 mean 6.23 vs median 0.40), so mean-raking ~doubles global deaths vs unadjusted while median-raking shrinks it ~5× (3679 / 1841 / 378 @2050 ssp245 for mean / unadj / median). SR31 observed deaths are 0 across all of 2000–2023 under BOTH statistics, so the SR31 guard is still required for the mean variant (mean-raking does NOT sidestep the hard-zero).
**Revisit if:** a canonical deliverable statistic is chosen — fold the winner (stat + guard) into the promoted `direct_deaths.{parquet,nc}` and note it here; ties into the still-open SR-31 guard decision above.

## 2026-07-14: Vintage contract for pipeline reruns; baseline tag `cycle-20260608`
**Decision:** Two more full pipeline cycles (new input data each) will run under a single-vintage contract: one string `<V>` = `YYYYMMDD` of ingest (suffix `_v2` etc. for same-day collisions), used verbatim in every stage directory — `00-data/<V>/` (input.parquet + is_island.parquet), `01-preliminary/<V>/`, `02-evaluate/<V>_refit/`, `02-evaluate/<V>_survivors/` + `scripts/survivors_<V>.json`, `01-refined/<V>/` + `02-evaluate/<V>_refined/`, `02-evaluate/<V>_final/`, `03-draws/<V>/<mid>/`, `04-predict/<V>/<mid>_{a0,a1,blend}/`. The 20260608 suffix set (`_refit`, `_survivors`, `_refined`, `_final`) is kept verbatim — even though `_refit` is now redundant — so repointing the four diagnostics notebooks to a new vintage is a global find/replace of the vintage string (plus re-deriving the final notebook's winner mids). Each cycle starts from a clean committed tree and closes with an annotated tag `cycle-<V>`. Tag `cycle-20260608` marks the code+notebook state of the completed 20260608 cycle (shipped-deliverable lineage). Ingest is parameterized (`--source-csv`, `--vintage`) so no per-cycle script edits; `build_draws.py --folds-path` is required (stale `20260608_final` default removed) to prevent silent cross-vintage fold misalignment.
**Why:** The orchestrators are already fully path-parameterized, so the vintage lives entirely in caller-supplied paths — the contract makes that discipline explicit. The known hazards to a rerun were: `current`-symlink drift (ingest auto-repoints `00-data/current`, several defaults follow it while others stay pinned), the edit-per-vintage hardcoded `SOURCE_CSV`, and the `build_draws` data-follows-current / folds-pinned-to-20260608 default mismatch (only a row-count guard between it and silent misalignment).
**Revisit if:** A cycle needs to vary something other than input data (e.g. covariate vintage only, like 20260706's SDI swap) — then the predict vintage decouples from the data vintage and the mapping must be recorded here; or if the notebook-per-cycle mechanism changes after the pending notebook reorganization discussion.

## 2026-07-14: Rerun vintages 20260714_v{1985,1990,1995} ingested from PSH_20260711 CSV
**Decision:** The rerun cycles start from three sibling vintages of the new source CSV (`ibtracs_stage4b_pafs_admin0_with_deaths_PSH_20260711_sdi_isisland.csv`, 3,882 raw rows = the previous 3,621 plus 261 rows flagged by the new `no_wind_exposure` column). All three use `--drop-no-wind-exposure` (new ingest flag; keeps only `no_wind_exposure == False`, hard-errors if the column parses non-boolean) and differ only in `--min-year`: `20260714_v1985` (3,024 rows, 1985–2023), `20260714_v1990` (2,658 rows, 1990–2023), `20260714_v1995` (2,278 rows, 1995–2023). `FILTER_MAX_YEAR` stays 2023 (the new CSV also ends at 2023). `00-data/current` → `20260714_v1995` (last ingested).
**is_island caveat:** `is_island.parquet` for all three was **copied from `00-data/20260608/`**, not re-pulled: no conda env on the node still has `db_queries` (the documented `GBD_shared_functions` env no longer does; a full env sweep found none). The covariate is a static (location_id, is_island) lookup pinned to covariate 2608 / release 16, so the copy is content-identical to a fresh pull (905 rows, int64/int8 schema verified).
**Why:** Bobby's spec for the two-to-three rerun cycles: sensitivity of the model to the training-window start year, on data that excludes no-wind-exposure rows.
**Revisit if:** `db_queries` becomes installable again — re-run `02_prepare_is_island.py` for the three vintages if pull-provenance matters; or if a winning min-year is chosen and the losing vintages are cleaned up.

## 2026-07-14: Intermediate decision for the 20260714 vintages; refined grids built (4 thresholds)
